...
def propose_fix(run_id: str, root_cause: str) -> str:
    ...
    if "knowledge gap" in rc or "similarity" in rc or "insufficient indexed" in rc:
        ...
        fix_action = "Index additional PRs to expand the knowledge base"
        fix_params = {"tool": "index_repo_prs", "repo": "tiangolo/fastapi", "count": 30}
        ...
        fix_type = "index_more_data"
        fix_action = "Agent exhausted hops — index more context to reduce tool dependency"
        fix_params = {"tool": "index_repo_prs", "repo": "tiangolo/fastapi", "count": 50}

    elif "routing shift" in rc:
        fix_type = "index_more_data"
        fix_action = "Memory routing changed — re-index to restore high-confidence path"
        fix_params = {"tool": "index_repo_prs", "repo": "tiangolo/fastapi", "count": 20}
...
def verify_fix(original_run_id: str, new_run_id: str) -> str:
    ...
    if not original or not new_run:
        return json.dumps({"status": "error", "message": "One or both run IDs not found."})
